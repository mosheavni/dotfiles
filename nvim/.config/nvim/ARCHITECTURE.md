...
- A second wave of _non-plugin_ `user.*` modules is deferred even further,
...
1. Registers a **`PackChanged` autocmd** _before_ the first `vim.pack.add()`
...
spec file _installs and adds the plugin to the runtimepath_; calling the returned
function _configures_ it.
...
eagerly. Only part of their _setup_ is deferred, via the separate `.eager()` /
...
`require 'user.pack'` does not _run_ the deferred batch — `vim.schedule` only
...
autocmd. So the consumer is registered _after_ the producer in source order but
comfortably _before_ the event is ever emitted.
...
| Module            | Purpose                                                                                                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `float.lua`       | Generic reusable floating-window primitive (buffer/window caching, refresh/close/toggle, sizing helpers). Used by `pack/float` and `:ParseCert`.                            |
...
| `tree.lua`            | `nvim-tree.lua`                                                                                                                                                            | nvim-tree setup and `on_attach` keymaps (including sort cycling and the `Z` extract mapping). Deferred.                                                                                                                                                                                |
...
- **Shared UI primitives.** Floats go through `user.float`; statusline colors
  come from `user.colorscheme.palette`.
