...
function kubedebug-ng() {
  ...
  local ctx namespace image docker_exe pod_name service_account choice
  local pod_name_ts=false host_network=false host_pid=false pull_policy overrides
  local -a kubectl_args ns_lines sa_lines
  local ns_list extra_args line sa_display extras_display hn_display hp_display ts_display
  ...
  while true; do
    ...
    if ((${#kubectl_args[@]} == 0)); then
      ...
      extras_display="${kubectl_args[*]}"
    fi
    [[ $host_network == true ]] && hn_display=yes || hn_display=no
    [[ $host_pid == true ]] && hp_display=yes || hp_display=no
    [[ $pod_name_ts == true ]] && ts_display=yes || ts_display=no
...
EOF
    ...
    Namespace)
      ...
      ;;
    Image)
      local prev_image=$image
      image=$(gum choose --header "Debug image" \
      ...
      if [[ $image == "Custom..." ]]; then
        ...
        image=$(gum input --header "Custom image" --placeholder "image:tag" --value "$custom_default") || continue
        [[ -z $image ]] && continue
      fi
      ;;
    Shell)
      if [[ $image == nicolaka/netshoot* ]]; then
        docker_exe=$(gum choose --header "Shell" --selected "$docker_exe" bash zsh sh) || continue
      else
        docker_exe=$(gum input --header "Executable" --value "$docker_exe") || continue
        [[ -z $docker_exe ]] && continue
      fi
      ;;
    "Pod name")
      pod_name=$(gum input --header "Pod name" --value "$pod_name") || continue
      [[ -z $pod_name ]] && continue
      ;;
    "Timestamp suffix")
      [[ $pod_name_ts == true ]] && pod_name_ts=false || pod_name_ts=true
      ;;
    "Service account")
      choice=$(gum choose --header "Service account" "(default)" "Pick from list...") || continue
      if [[ $choice == "(default)" ]]; then
        service_account=
        continue
      fi
      if ! sa_lines=$(gum spin --spinner dot --title "Fetching service accounts..." --show-stdout -- \
      ...
      service_account=$(printf '%s\n' "${sa_lines[@]}" |
        gum filter --strict \
          --header "Service account ($namespace)" \
          --placeholder "type to filter..." |
        awk '{print $1}')
      ;;
    hostNetwork)
      [[ $host_network == true ]] && host_network=false || host_network=true
      ;;
    hostPID)
      [[ $host_pid == true ]] && host_pid=false || host_pid=true
      ;;
    "Pull policy")
      pull_policy=$(gum choose --header "Image pull policy" \
        --selected "$pull_policy" \
        IfNotPresent Always Never) || continue
      ;;
    "Extra flags")
  ...
  overrides=
  if [[ -n $service_account ]] || $host_network || $host_pid; then
    overrides=$(jq -nc \
  ...
  clear
  gum style --foreground 212 --border rounded --padding "0 1" \
    "${run_cmd[*]}"
  gum confirm "Launch debug pod?" || return 0
