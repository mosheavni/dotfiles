...
# Sync MCP servers configuration from mcphub to Claude settings
# Source of truth: .config/mcphub/servers.json (edit via nvim UI)
# Target: .claude/settings.json (preserves alwaysThinkingEnabled)

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_FILE="$SCRIPT_DIR/.config/mcphub/servers.json"
TARGET_FILE="$SCRIPT_DIR/.claude/settings.json"
...
# Update target with synced mcpServers (preserves all other fields like alwaysThinkingEnabled)
jq --argjson mcp "$SOURCE_MCP" '.mcpServers = $mcp' "$TARGET_FILE" > "$TARGET_FILE.tmp"

# Replace target file
mv "$TARGET_FILE.tmp" "$TARGET_FILE"

echo "✓ Synced MCP servers from mcphub to Claude settings"
